...
class RAP:
    def __init__(self, args: RAPConfiguration, key: np.DeviceArray):
        ...
        self.tracker = zCDPTracker(args.epsilon, args.delta, args.epochs, args.top_q, args.num_points)
    ...
    def __log_progress(self, iteration: int = -1) -> None:
        ...
        logging.info("Loss: {:.5f}, L2 Error {:.5f} Max Error {:.5f}".format(
            loss, l2_error, max_error))

    def __compute_initial_dataset(self, selection: SyntheticInitializationOptions,
                                  key: np.DeviceArray) -> np.DeviceArray:
        """
        ...
        if selection is SyntheticInitializationOptions.RANDOM:
            ...
            return ((interval.projection_max - interval.projection_min) * random_initial) + interval.projection_min
        elif selection is SyntheticInitializationOptions.RANDOM_BINOMIAL:
            return np_orig.array(
                [np_orig.random.binomial(1, p=self.args.probs) for _ in range(self.args.num_generated_points)],
                dtype=float)
        else:
            raise ValueError("Supported selections are ",
                             [member.value for _, member in SyntheticInitializationOptions.__members__.items()])
    ...
    def __initialize_synthetic_dataset(self, key: np.DeviceArray):
        ...
        if self.args.projection_interval:
            ...
            return self.__compute_initial_dataset(SyntheticInitializationOptions.RANDOM_INTERVAL, key)
        else:
            if self.args.initialize_binomial:
                return self.__compute_initial_dataset(SyntheticInitializationOptions.RANDOM_BINOMIAL, key)
            else:
                return self.__compute_initial_dataset(SyntheticInitializationOptions.RANDOM, key)

    def __compute_composition(self) -> Tuple[float, float]:
        # advanced composition
        epsilon_p = self.args.epsilon / \
                    np.sqrt(2 * (2 * self.args.epochs) * np.log((2 * self.args.num_points) ** 2))
        delta_p = (self.args.num_points ** -2 - (2 * self.args.num_points)
                   ** -2) / (2 * self.args.epochs)
        return epsilon_p, delta_p

    def __jit_loss_fn(self, statistic_fn: Callable[[np.DeviceArray], np.DeviceArray]) -> Callable[
        [np.DeviceArray, np.DeviceArray], np.DeviceArray]:

        ord_norm = norm_mapping[self.args.norm]

        @jit
        def compute_loss_fn(synthetic_dataset: np.DeviceArray, target_statistics: np.DeviceArray) -> np.DeviceArray:
            if self.args.norm is Norm.LOG_EXP:
                return np.log(np.exp(
                    statistic_fn(synthetic_dataset) - target_statistics).sum()) + self.args.lambda_l1 * np.linalg.norm(
                    synthetic_dataset, 1)
            else:
                return np.linalg.norm(statistic_fn(synthetic_dataset) - target_statistics,
                                      ord=ord_norm) + self.args.lambda_l1 * np.linalg.norm(synthetic_dataset, 1)

        return compute_loss_fn

    def __get_update_function(self, learning_rate: float, optimizer: Callable[..., optimizers.Optimizer],
                              loss_fn: Callable[[np.DeviceArray, np.DeviceArray], np.DeviceArray]) -> Tuple[
    ...
    def train(self, dataset: np.DeviceArray, k_way_attributes: Any, key: np.DeviceArray) -> None:
        ...
        for epoch in range(self.args.epochs):
            query_errs = np.abs(self.args.statistic_function(self.D_prime) - true_statistics)
            logging.info("Average error: {}, Error standard deviation: {}".format(
            ...
            logging.info("DP budget per epoch  -- Epsilon: {}, Delta: {}".format(zCDPTracker.convert_zcdp_to_eps_delta(
                epoch_total_budget, self.args.delta), self.args.delta))
            ...
            target_statistics = np.concatenate(
                [target_statistics,
                 self.tracker.gaussian_mechanism(key, current_statistic_fn, dataset, query_answer_budget)])
            ...
            curr_statistic_fn = self.args.preserve_subset_statistic(np.asarray(curr_queries))
            ...
            optimizer_learning_rate = self.args.optimizer_learning_rate  # * 2 ** (-epoch)
            update, opt_state = self.__get_update_function(
                optimizer_learning_rate, optimizers.adam, loss_fn)

            for iteration in range(self.args.iterations):

                self.D_prime, opt_state, loss = update(self.D_prime, target_statistics, opt_state)
                if self.feats_idx:
                     self.D_prime = sparsemax_project(
                         self.D_prime, self.feats_idx)
                ...
                self.max_errors.append(np.max(np.absolute(true_statistics - all_synth_statistics)))
                self.l2_errors.append(np.linalg.norm(true_statistics - all_synth_statistics, ord=2))
        ...
        logging.info("DP budget used  -- Epsilon: {}, Delta: {}".format(
            zCDPTracker.convert_zcdp_to_eps_delta(total_zcdp_used, self.args.delta), self.args.delta))
    ...
    def generate_rounded_dataset(self, key, oversample=None):
        ...
        return randomized_rounding(
            D=self.D_prime, feats_idx=self.feats_idx, key=key,
