...
def preserve_statistic(queries):
    ...
    def compute_statistic(D):
        return np.concatenate([
                D.shape[0] - np.einsum(eins_string, *[(1-D[:, idx_q]) for idx_q in q]).flatten() for q in queries
        ]) / D.shape[0]
...
def preserve_subset_statistic(queries):
    @jit
    def compute_statistic(D):
        np.concatenate([(1 - np.prod(1-D[:, q], 2)).sum(0) for q in np.array_split(queries, 10)])
        return np.concatenate([
