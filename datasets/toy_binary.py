...
class ToyBinary(Dataset):
    ...
    def randomKway(self, num_kways=24, k=3, seed=0):
        ...
        proj = [p for p in itertools.combinations(self.df.columns, k) if self._get_size_domain(p) <= total]
    ...
    def project_feats(self, feats=None):
        ...
        bins_size_array = [(size_bin, np.digitize(self.df[col], range(size_bin+1), right=True)) for col, size_bin in feats_domain.items()]

        # perform one-hot-encoding of all features and stack them into a numpy matrix
        bin_dataset = np.hstack([np.eye(size_bin)[bin_array] for size_bin, bin_array in bins_size_array])
        return bin_dataset
