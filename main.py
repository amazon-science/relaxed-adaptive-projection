...
parser.add_argument('--config-file', '-c', required=False, is_config_file=True, help='Path to config file')
parser.add_argument('--num-dimensions', '-d', type=int, default=2, dest='d', help='Number of dimensions in the '
                                                                                  'original dataset. Does not need to '
                                                                                  'be set when consuming csv files ('
                                                                                  'default: 2)')

parser.add_argument('--num-points', '-n', type=int, default=1000, dest='n', help='Number of points in the original '
                                                                                 'dataset. Only used when generating '
                                                                                 'datasets (default: 1000)')

parser.add_argument('--num-generated-points', '-N', type=int, default=1000, dest='n_prime', help='Number of points to '
                                                                                                 'generate (default: '
                                                                                                 '1000)')

parser.add_argument('--epsilon', type=float, default=1, help='Privacy parameter (default: 1)')
parser.add_argument('--delta', type=float, default=None, help='Privacy parameter (default: 1/n**2)')
parser.add_argument('--iterations', type=int, default=1000, help='Number of iterations (default: 1000)')

parser.add_argument('--save-figures', type=bool, default=False, dest='save_fig', help='Save generated figures')
parser.add_argument('--no-show-figures', type=bool, default=False, dest='no_show_fig', help='Not show generated figures'
                                                                                            'during execution')

parser.add_argument('--ignore-diagonals', type=bool, default=False, dest='ignore_diag', help='Ignore diagonals')
parser.add_argument('--data-source', type=str, choices=data_sources.keys(), default='toy_binary',
                    dest='data_source', help='Data source used to train data generator')
parser.add_argument('--read-file', type=bool, default=False, help='Choose whether to regenerate or read data from file '
                                                                  'for randomly generated datasets')

parser.add_argument('--use-data-subset', type=bool, default=False, dest='use_subset', help='Use only n rows and d '
...
parser.add_argument('--destination_path', type=str, default='figures/', dest='destination', help='Location to save '
                                                                                                 'figures and '
                                                                                                 'configuration')
parser.add_argument('--seed', type=int, default=0, help='Seed to use for random number generation')
parser.add_argument('--statistic-module', type=str, default='statistickway',
...
parser.add_argument('--workload', type=int, default=64, help='workload of marginals (default 64)')
...
parser.add_argument('--initialize_binomial', type=bool, default=False, help='Initialize with 1-way marginals')
parser.add_argument('--lambda-l1', type=float, default=0,
                    help='L1 regularization term (default: 0)')
parser.add_argument('--stopping-condition', type=float, default=10 ** -7,
...
parser.add_argument('--top-q', type=int, default=50, help='Top q queries to select (default q=500)')
parser.add_argument('--epochs', type=int, default=10,
                    help='Number of epochs (default: 100)')
# parser.add_argument('--select-T', type=float, default=0.1,
#                     help='Threshold T on absolute errors (default 0.1)')
parser.add_argument('--csv-path', type=str, default='results', dest='csv_path',
...
parser.add_argument('--norm', type=str, choices=['Linfty', 'L2', 'L5', 'LogExp'],
                    default='L2', help='Norm to minimize if using the optimization paradigm (default: L2)')

parser.add_argument('--categorical-consistency', action='store_true',
                    help='Enforce consistency categorical variables')
parser.add_argument('--measure-gen', action='store_true',
                    help='Measure Generalization properties')

parser.add_argument('--oversamples', type=str, default=None,
                    help='comma separated values of oversamling rates (default None)')

args = parser.parse_args()
if args.silent and args.verbose:
    raise ValueError('You cannot choose both --silent and --verbose. These are conflicting options. Choose at most one')
...
if args.all_queries:
    logging.warning("--all-queries option has been chosen. No selection of top queries will occur")
...
dataset = data_sources[args.data_source](args.read_file, args.filepath, args.use_subset,
                                         args.n, args.d)
...
args.epochs = min(args.epochs,
                  np.ceil(len(true_statistics) / args.top_q).astype(np.int32)) if not args.all_queries else 1
...
algorithm_configuration = RAPConfiguration(num_points=args.n,
    ...
    if args.oversamples:
        ...
        for oversample in args.oversamples.split(","):
            ...
            for rep in range(repetitions):
                Dprime_ohe = rap.generate_rounded_dataset(key, oversample=int(oversample))
            ...
            oversampling_errors[oversample] = [
                ('max_error', float(np.mean(max_final_ohe)), float(np.std(max_final_ohe))),
        ...
        pd.DataFrame.from_dict(oversampling_errors, orient='index').to_csv(
            os.path.join(args.csv_path, 'rounded_oversample_errors_{}_{}_{}_eps{}_T{}_K{}.csv'.format(
                args.data_source, args.workload, args.k, args.epsilon, args.epochs, args.top_q)),
        )
        ...
        all_synth_statistics_ohe = all_statistic_fn(Dprime_ohe)
        max_final_ohe = np.max(np.absolute(true_statistics - all_synth_statistics_ohe))
        l1_final_ohe = np.linalg.norm(true_statistics - all_synth_statistics_ohe, ord=1) / float(args.workload)
...
if args.measure_gen:
    ...
    max_base_2 = np.max(np.absolute(true_statistics_2 - np.zeros(true_statistics_2.shape)))
    l2_base_2 = np.linalg.norm(true_statistics_2 - np.zeros(true_statistics_2.shape), ord=2)
...
if args.csv_path:
    ...
    names = ["epsilon", "max_error", "l2_error", "max_base", "l2_base", "npoints", "norm", "l1_reg", "top_q", "epochs"]
    res = [args.epsilon, max_final, l2_final, max_base, l2_base, args.n_prime, args.norm, args.lambda_l1, args.top_q,
           args.epochs]
    ...
    file_name = os.path.join(args.csv_path,
                             fn_prefix + "_adaptive_{}_{}_{}.csv".format(args.data_source, args.workload, args.k))
    ...
    df_res.sort_values(by=['epsilon', "norm", "npoints", "l1_reg"], ascending=False). \
        to_csv(file_name, index=False)

    pd.DataFrame(rap.D_prime).to_csv(
        os.path.join(args.csv_path, 'synthetic_data_{}_{}_{}.csv'.format(args.data_source, args.workload, args.k)),
        index=False
    )
    ...
    if args.categorical_consistency:
        ...
        pd.DataFrame(data=Dprime_cat, columns=list(dataset.domain.keys())).to_csv(
            os.path.join(args.csv_path, 'rounded_synthetic_data_{}_{}_{}_eps{}_T{}_K{}.csv'.format(
                args.data_source, args.workload, args.k, args.epsilon, args.epochs, args.top_q)),
            index=False
        )
