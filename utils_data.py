...
def init_D_prime(selection, n_prime, d, D=False, interval=None):
    ...
    if selection == 'random':
        ...
        raise ValueError("Supported selections are 'random', 'randomunit', and 'near_origin'")
    return Dprime
...
def GDP_to_DP(mu, delta):
    ...
    lower = 0.0
    upper = 500.0
    while (abs(deltaval((lower+upper)/2)-delta) >= delta/10 and (lower+upper/2) > 0): # binary search to get within (1+-1/10)*delta
...
def DP_to_GDP(epsilon, delta):
    ...
    mid = (lower + upper) / 2
    while abs(GDP_to_DP(mu=mid, delta=delta) - epsilon) >= (epsilon * 10**-2) and mid > 0:
...
def jit_loss_fn(statistic_fn, norm=None, lambda_l1=0):
    ...
    def compute_loss_fn(Dprime, target_statistics):
        if norm == 'LogExp':
            return np.log(np.exp(statistic_fn(Dprime) - target_statistics).sum()) + lambda_l1 * np.linalg.norm(Dprime, 1)
        else:
            return np.linalg.norm(statistic_fn(Dprime) - target_statistics, ord=ord_norm) + lambda_l1 * np.linalg.norm(Dprime, 1)
...
def project_gumbel_softmax(D, feats_idx, key, temperature=1.0):
    ...
    return np.hstack(
        gumbel_softmax(D[:, q], key, temperature) for q in feats_idx
        )
...
def sparsemax_project(D, feats_idx):
    ...
    return np.hstack(
        sparsemax(D[:, q]) for q in feats_idx
        )
...
def randomized_rounding(D, feats_idx, key, oversample=1):
    ...
    return np.hstack(
                np.vstack(nn.one_hot(random.choice(key, a=len(probs), shape=(oversample, 1), p=probs).squeeze(), len(probs))
                    for probs in D[:, feat]
                    )
